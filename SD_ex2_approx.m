...
%%
save('Exp2_Jacob.mat','Jacob_F_V','Der_F_del1','Der_F_del2','f*');
%%
clear all
load('Exp2_Jacob.mat')
...
%%
%%=========================================================================
%%=  Setting the initial and boundary condition
%%=
%%=========================================================================
%%==Using mesh size 0.005
%load('Ex2_sol.mat')
%ind1=11;ind2=11;
...
ind1=1; ind2=1;
delta1=(-0.0:0.001:0.049);
delta2=(-0.0:0.001:0.05);
L1=length(delta1);
L2=length(delta2);
sol=zeros(L1,L2,15);
...
for i=2:L1
    for j=2:L2
        if j<=2
            ...
            hs11(i-1,j)=s_11(i-1,j);
            hs12(i-1,j)=s_12(i-1,j);
            hs13(i-1,j)=s_13(i-1,j);
            hs22(i-1,j)=s_22(i-1,j);
            hs23(i-1,j)=s_23(i-1,j);
            hs33(i-1,j)=s_33(i-1,j);
        elseif j>=L2-1
            hm(i-1,j)=m_n(i-1,j);
            hy1(i-1,j)=y_1(i-1,j);
            hy2(i-1,j)=y_2(i-1,j);
...
%         
        ...
        s_11(i,j)=s_11(i-1,j)+del_1*G(4)-del_1/del_2*(hx11(i-1,j)-hx11(i-1,j-1));
        s_12(i,j)=s_12(i-1,j)+del_1*G(5)-del_1/del_2*(hx12(i-1,j)-hx12(i-1,j-1));
        s_13(i,j)=s_13(i-1,j)+del_1*G(6)-del_1/del_2*(hx13(i-1,j)-hx13(i-1,j-1));
        s_22(i,j)=s_22(i-1,j)+del_1*G(7)-del_1/del_2*(hx22(i-1,j)-hx22(i-1,j-1));
        s_23(i,j)=s_23(i-1,j)+del_1*G(8)-del_1/del_2*(hx23(i-1,j)-hx23(i-1,j-1));
        s_33(i,j)=s_33(i-1,j)+del_1*G(9)-del_1/del_2*(hx33(i-1,j)-hx33(i-1,j-1));
        x_11(i,j)=x_11(i-1,j)+del_1*G(9)-del_1/del_2*(hs11(i-1,j)-hs11(i-1,j-1));
        x_12(i,j)=x_12(i-1,j)+del_1*G(11)-del_1/del_2*(hs12(i-1,j)-hs12(i-1,j-1));
        x_13(i,j)=x_13(i-1,j)+del_1*G(12)-del_1/del_2*(hs13(i-1,j)-hs13(i-1,j-1));
        x_22(i,j)=x_22(i-1,j)+del_1*G(13)-del_1/del_2*(hs22(i-1,j)-hs22(i-1,j-1));
        x_23(i,j)=x_23(i-1,j)+del_1*G(14)-del_1/del_2*(hs23(i-1,j)-hs23(i-1,j-1));
        x_33(i,j)=x_33(i-1,j)+del_1*G(15)-del_1/del_2*(hs33(i-1,j)-hs33(i-1,j-1));
        X=[x_11(i,j) x_12(i,j) x_13(i,j);...
           x_12(i,j) x_22(i,j) x_23(i,j);...
           x_13(i,j) x_23(i,j) x_33(i,j)];
        S=[s_11(i,j) s_12(i,j) s_13(i,j);...
           s_12(i,j) s_22(i,j) s_23(i,j);...
           s_13(i,j) s_23(i,j) s_33(i,j)];
        X_min=min(eig(X)); 
        S_min=min(eig(S));
